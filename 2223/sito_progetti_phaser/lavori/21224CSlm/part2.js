class part2 extends Phaser.Scene {
    constructor() {
        super("part2")
    }

    init(e) { // Parametri passati dal cambio della scena
    ...
    preload() {
        this.load.image("bg2", "./assets/Backgrounds/Battleground2.png")
    }

    create() {
        this.enemies_number = 3
        this.bg = this.add.image((960 / 2), (540 / 2), "bg2")
        ...
        for (let i = 0; i < this.enemies_number; i++) {
            ...
            this.enemies[i] = this.physics.add.sprite(800, y, "t2I0").setSize(480, 500).setOffset(550, 380)
            this.enemies[i].displayWidth = 350
            this.enemies[i].displayHeight = 200
            this.enemies[i].life = 95
            this.enemies[i].died = false
            this.enemies[i].frames = [`t2I`, `t2R`, `t2A`, `t2H`, `t2D`]
